...
import { InstructorGetModuleUseCase } from './get-module-use-case'
...
describe('InstructorGetModuleUseCase', () => {
	let moduleRepo: ModuleRepository
	let courseRepo: CourseRepository
	let sut: InstructorGetModuleUseCase

	beforeEach(() => {
		moduleRepo = mock(ModuleRepository)
		courseRepo = mock(CourseRepository)
		sut = new InstructorGetModuleUseCase({
	...
	it('should return success with module when instructor is the owner', async () => {
		const module = await Module.create({
			idGenerator: new FakeIdGenerator(),
			input: { courseId: UniqueId('course-id'), title: 'Module 1', order: 1 },
		})
